...
with(x, {with(x,plot(datetime,globalactivepower,type = "l",xlab = "", lwd = 1.5, 
                     ylab = "Global Active Power"))
        with(x,plot(datetime,voltage,type = "l",ylab = "Voltage",lwd = 1.5))
        {with(x,plot(datetime,submetering1,type = "n", xlab = "" , 
                     ylab = "Energy sub metering"))
                lines(x$datetime,x$submetering1,col = "black", lwd = 1.5)
                lines(x$datetime,x$submetering2,col = "red", lwd = 1.5)
                lines(x$datetime,x$submetering3,col = "blue", lwd = 1.5)
                legend("topright",lty = "solid", col = c("black","red","blue"), 
                       legend = c("Sub_metering_1","Sub_metering_2","Sub_metering_3"),
                       bty = "n")
        }
        with(x,plot(datetime,globalreactivepower,type = "l", lwd = 1.5, 
